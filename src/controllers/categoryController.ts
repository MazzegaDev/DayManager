...
export default class CategoryController {
	...
	async createCategory(req: Request, res: Response): Promise<Response> {
		try {
			...
			return res.status(201).json({ msg: "Nova categoria criada!", data: created });
		} catch (error: any) {
			console.log(error);
			if (error.statusCode) {
	...
	async listUserCategory(req: Request, res: Response): Promise<Response> {
		try {
			const user_id = req.user.user_id;
	...
	async findById(req: Request<CategoryParams>, res: Response): Promise<Response> {
		try {
	...
	async updateCategory(req: Request, res: Response): Promise<Response> {
		try {
			...
			const updated: CategoryUpdateDto = await this.cateServ.updateCategory(obj);

			return res.status(200).json({ msg: "Dados da categoria alterados!", data: updated });
		} catch (error: any) {
			console.log(error);
			if (error.statusCode) {
				return res.status(error.statusCode).json(error.message);
	...
	async deleteCategory(req: Request<CategoryParams>, res: Response): Promise<Response> {
		try {
			const { cate_id } = req.params as CategoryParams;

			const deleted: Category = await this.cateServ.deleteCategory(cate_id);

			return res.status(200).json({ msg: "Categoria deletada!", data: deleted });
		} catch (error: any) {
			console.log(error);
			if (error.statusCode) {
				return res.status(error.statusCode).json(error.message);
