...
export default class TaskController {
	...
	async createTask(req: Request, res: Response): Promise<Response> {
		try {
			let { task_name, task_priority, cate_id, task_day } = req.body as TaskReqParamsCreateDto;
			...
			return res.status(201).json({ msg: "Nova tarefa criada!", data: created });
		} catch (error: any) {
			console.log(error);
			if (error.statusCode) {
	...
	async listPerCategory(req: Request<CategoryParams>, res: Response): Promise<Response> {
		try {
			const { cate_id } = req.params as CategoryParams;
			const user_id = req.user.user_id;

			const list: TaskIncludeDto[] = await this.taskServ.listPerCategory(cate_id, user_id);
	...
	async findTaskById(req: Request<TaskParams>, res: Response): Promise<Response> {
		try {
	...
	async deleteTask(req: Request<TaskParams>, res: Response): Promise<Response> {
		try {
			const { task_id } = req.params;
