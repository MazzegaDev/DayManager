...
export default class UserController {
   ...
   async createUser(req: Request, res: Response): Promise<Response> {
      try {
         ...
         return res
            .status(201)
            .json({ msg: "Novo usuario cadastrado!", data: created });
      } catch (error: any) {
         console.log(error);
         if (error.statusCode) {
   ...
   async updateUser(req: Request, res: Response): Promise<Response> {
      try {
         const { user_id, user_email, user_name, user_pass } =
         ...
         return res
            .status(201)
            .json({ msg: "Dados do usuario alterados!", data: created });
      } catch (error: any) {
         console.log(error);
         if (error.statusCode) {
            return res.status(error.statusCode).json(error.message);
   ...
   async deleteUser(
      ...
      try {
         ...
         return res
            .status(200)
            .json({ msg: "Usuario deletado!", data: deleted });
      } catch (error: any) {
         console.log(error);
         if (error.statusCode) {
            return res.status(error.statusCode).json(error.message);
