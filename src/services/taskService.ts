...
export default class TaskService {
	...
	async createTask(data: TaskInputDto): Promise<Task> {
		let { task_name, task_priority, user_id, cate_id, task_day } = data;

		const findedCategory: Category | null = await this.cateRepo.findById(cate_id);
		if (!findedCategory) {
	...
	async listPerCategory(cate_id: string | number, user_id: number): Promise<TaskIncludeDto[]> {
		...
		let list: TaskIncludeDto[] = await this.taskRepo.listPerCategory(idCate, user_id);
	...
	async findTaskById(task_id: string): Promise<TaskIncludeDto> {
		...
		const finded: TaskIncludeDto | null = await this.taskRepo.findTaskById(id);
	...
	async updateTask(data: TaskUpdateDto): Promise<Task> {
		let { task_id, task_name, task_priority, task_status, user_id, cate_id, task_day } = data;
		...
		if (!updated) {
			throw new AppError("Não foi possivel atualizar os dados da tarefa", 500);
		}
